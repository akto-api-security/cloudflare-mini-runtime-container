...
export class MiniRuntimeContainerMotleyFool extends Container {
...
type Environment = {
  readonly MINI_RUNTIME_CONTAINER_MOTLEY_FOOL: DurableObjectNamespace<MiniRuntimeContainerMotleyFool>
  readonly AKTO_TRAFFIC_QUEUE_MOTLEY_FOOL: Queue<any>
}
const app = new Hono<{
...
export default {
  ...
  async queue(batch: MessageBatch<any>, env: Environment) {
    ...
    for (let i = 0; i < messages.length; i += 5) {
      ...
      const containerInstance = getRandom(env.MINI_RUNTIME_CONTAINER_MOTLEY_FOOL, INSTANCE_COUNT)
      const containerId = env.MINI_RUNTIME_CONTAINER_MOTLEY_FOOL.idFromName(`/container/${containerInstance}`)
      const container = env.MINI_RUNTIME_CONTAINER_MOTLEY_FOOL.get(containerId)
      ...
      try {
        ...
        if (res.ok) {
          ...
          for (const m of batchSlice) {
            await env.AKTO_TRAFFIC_QUEUE_MOTLEY_FOOL.send(m.body)
          }
        }
      } catch (err) {
